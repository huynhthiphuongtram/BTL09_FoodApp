...
from .serializers import (FoodCategorySerializer, FoodSerializer, FoodDetailsSerializer,
                          ...
                          StoreSerializer, OrderDetailSerializer, MenuSerializer)
...
class MenuViewSet(viewsets.ViewSet, generics.ListAPIView, generics.RetrieveAPIView):
    ...
    def details(self, request, pk):
        ...
        details = c.food_set.filter(active=True)
        kw = request.query_params.get('kw')
        if kw:
            details = details.filter(name__icontains=kw)

        return Response(MenuSerializer(details, many=True, context={'request': request}).data)
